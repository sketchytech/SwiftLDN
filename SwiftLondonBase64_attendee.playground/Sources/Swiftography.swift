        ...
        if let url = NSBundle.mainBundle().URLForResource("sha3", withExtension: "js")
{
    ...
    public static func SHA256Hmac(str:String, key:String) -> String? {
        
        if let url = NSBundle.mainBundle().URLForResource("hmac-sha256", withExtension: "js") {
            var a:JSValue?
            do {
                let js = try String(contentsOfURL: url, encoding: NSUTF8StringEncoding)
                // First a context and JS virtual machine is created
                ...
                // generate JSValue
                a = context.evaluateScript("CryptoJS.HmacSHA256('\(str)', '\(key)')")
                print(a)
